...
>
> 我们提出了一种新的架构，我们称之为3D-Convolutional LSTM（3D-LSTM）。该网络由一组具有受限连接的结构化LSTM单元组成。 3D-LSTM单元以3D网格结构在空间上分布，每个单元负责重建最终输出的特定部分
>
> 在3D网格内部，有𝑁×𝑁×𝑁个3D-LSTM单位，其中N是3D-LSTM网格的空间分辨率。索引（i，j，k），每个3D-LSTM单元具有独立的隐藏状态ℎ𝑡。
>
> 通过去除冗余输出门，我们可以减少参数的数量。
>
...
3. dropout没有尝试 过拟合问题的使用了正则化项解决(loss函数的正则化)
可以尝试loss函数的正则化项的选择
